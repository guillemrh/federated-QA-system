...
class Retriever:
    ...
    def build_index(self) -> "Retriever":
        """
        Build a FAISS index from the loaded text chunks and store embeddings.
        """
        if not self.text_chunks:
        ...
        for chunk, emb in zip(self.text_chunks, embeddings):
            chunk["embedding"] = emb.tolist()  # store as list for JSON serializable
    ...
    def search(self, query: str, k: int = 2, return_distances: bool = True):
        ...
        return (distances[0], results, query_embedding[0].tolist()) if return_distances else (None, results, query_embedding[0].tolist())
